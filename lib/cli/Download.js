...
function download(type,fromUrl,toPath,cli,next) {
  ...
  downloadRepo(type,fromUrl,toPath,cli,localNext);
  return;
...
function downloadRepo(type,fromUrl,toPath,cli,next) {
  ...
  if(repoName) {
    ...
    constructRepoUrl(repoName,version,function(err,url) {
      if(!err && url) {
        download(type,url,toPath,cli,next);
      } else {
        next(err);
      }
    });
    ...
    next(new Error('B You need to provide a calipso repository project name - e.g. ElasticSearch'));
  }
...
function constructRepoUrl(repoName,version,next) {
  ...
  var repo = new Api("http://localhost:3001/repo/api/");
  ...
  repo.list({type:type,name:name,version:version},function(err,r) {
      if(err || !r) {
        next(err);
      } else {
        if(r.length === 1) {
          ...
          versions.forEach(function(v) {
              if(v.version === version) {
                next(null,v.url);
                versionMatched = true;
                console.log("Resolved ".cyan.bold + repoName.white + "@".white + v.version.white + " to ".cyan.bold + v.url + (v.version === "master" ? "" : "@" + v.version));
                return;
              }
          });
          if(!versionMatched) {
          ...
          next(new Error("There was an error locating that module, multiple entries returned"));
        }
      }
  });
