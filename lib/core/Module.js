    ...
    if(module.enabled && (depends || !module.fn.depends) && (last || !module.fn.last)) {
      ...
      module.fn.route(req, res, module, calipso.app, function(err, moduleName) {
        ...
        if(err) {
	 	...
	 	if(calipso.modules[moduleName].fn.config) {
...
function loadModules(next) {
  ...
  calipso.lib.fs.readdirSync(rootpath + 'modules').forEach(function(type){

    if(type != "README" && type != '.DS_Store') {  // Ignore the readme file and .DS_Store file for Macs

      calipso.lib.fs.readdirSync(rootpath + 'modules/' + type).forEach(function(moduleFolderName){
