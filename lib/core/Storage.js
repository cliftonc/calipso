  ...
  function proceed(err) {
    if (err) {
      ...
      var foundMiddleware = false, mw;
        ...
        (calipso.app._router.stack || calipso.app.stack).forEach(function (middleware, key) {
        if (middleware.handle.tag === 'session') {
          ...
          var mw = function session(req, res, next) {
            // Wrapper to default session as at least an empty object.
            expressSession(req, res, function () {
              ...
              Object.defineProperty(req.session, "user", {
                ...
                get: function() {
                  ...
                  if (user) {
                    return {
                      isAdmin: calipso.lib.user.isUserAdmin(user),
          ...
          mw.tag = 'session';
            (calipso.app._router.stack || calipso.app.stack)[key].handle = mw;
        }
      });
