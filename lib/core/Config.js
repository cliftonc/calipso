...
Config.prototype.init = function(next) {  
  if(typeof next !== 'function') next = function(err) { if(err) console.error(err.message) };
...
Config.prototype.load = function(next) {
  ...
  if(this.check()) {
    ...
    next(new Error("Unable to load configuration defined in " + this.env + ".json, there may be a problem with the default configuration in " + this.defaultConfig));
