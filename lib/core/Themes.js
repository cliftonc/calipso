    ...
    if(isCached) {
      ...
      processSection(req, res, section, templateName, layoutConfig, theme, function(err) {
         if(!err) {
           ...
           calipso.cacheService.set(cacheKey,{content:content},null,next);
         } else {
           next(err);
         }
      });
...
function cacheTheme(themeConfig, themePath, next) {
  ...
  if (themeConfig) {
    ...
    calipso.lib._.each(errorCodeTemplates, function(filename){
      templates.push({name: filename.match(/^\d{3}/)[0], templatePath: calipso.lib.path.join("templates","error",filename)});
    });
