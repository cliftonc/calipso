...
function CalipsoEventEmitter(options) {
  ...
  this.pre_emit = function(event,data,next) {
    ...
    if(this.events[event] && this.events[event].enabled) {      
      if(this.events[event].hookio && calipso.app.isCluster) {
        this.hook.emit('calipso::' + pre_prefix + event, data, cb);
        // cb(data);
      } else {
  ...
  this.post_emit = function(event,data,next) {
    ...
    if(this.events[event] && this.events[event].enabled) {      
      if(this.events[event].hookio && calipso.app.isCluster) {        
        this.hook.emit('calipso::' + post_prefix + event, data, cb); 
        // cb(data);
      } else {
  ...
  this.custom_emit = function(event, key, data, next) {
    ...
    if(this.events[event] && this.events[event][key]) {
      ...
      // Create a callback to track completion of all events (only if next exists)
      if(typeof next === "function") {
        var cb = createCallback(this.events[event][key].customListeners['#'],data,next);
      } else {
        var cb = function() {};
      }

      if(this.events[event].enabled) {                 
        if(this.events[event][key].hookio && calipso.app.isCluster) {
          this.hook.emit('calipso::' + event + '::' + key, data, cb); 
          // cb(data);
        } else {
