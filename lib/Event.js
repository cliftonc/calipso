...
function CalipsoEventEmitter() {
  ...
  // Emit an event
  this.pre_emit = function(event,options,next) {     
        
    // Create a callback to track completion of all events (only if next exists)
    if(typeof next === "function") {
      var cb = createCallback(this.events[event].preListeners['#'],options,next);    
    } else {
      var cb = function() {};
    }
    
    if(this.events[event] && this.events[event].enabled) {
      self.emitter.emit(pre_prefix + event,options,cb);
    }
        
  }
  
  // Emit an event
  this.post_emit = function(event,options,next) {
    
    // Create a callback to track completion of all events (only if next exists)
    if(typeof next === "function") {
      var cb = createCallback(this.events[event].postListeners['#'],options,next);
    } else {
      var cb = function() {};
    }
    
    if(this.events[event] && this.events[event].enabled) {
      self.emitter.emit(post_prefix + event,options,cb);
    }
    
  }
  
  // Create a curried callback function for use in the emit code
  function createCallback(total,options,callback) {
