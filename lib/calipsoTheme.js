      ...
      var theme = {
        ...
        render: function(req, res, next) {
          ...
          processTheme(req,res,layout,theme,function() {
            ...
            var content = cache[layout].template.call({}, res.bufferedOutput);
            next(content);
...
function processSection(req, res, section, themeName, layoutConfig, theme, next) {
  ...
  if(!layoutConfig.sections[section].menu) {
    ...
    // If we have a block section, pre-concatenate their content    
    ...
    if(res.menu[section]) {
      ...
      var menu = themeCache.template.call({}, themeOptions);
      res.bufferedOutput[section] += menu;
    }
    next();
    ...
    if(exists) {
      fs.readFile(templatePath, 'utf8', function(err,data) {
        if(!err) {
          ...
          // Add a watcher so that we can recompile any changes in a theme immediately, only watch once?
          if(!templateCache[template.name].watcher) {   
                     
            calipso.silly("Adding watcher for " + template.name);
            templateCache[template.name].watcher = true;
                        
            fs.watchFile(templatePath, { persistent: true, interval: 1000}, function(curr,prev) {
                
                if(templateCache[template.name].mtime != curr.mtime) {
                  
                  templateCache[template.name].mtime = curr.mtime;
                  
                  // TODO - for some reason we are getting 2 events, need to look into it.                  
                  calipso.silly("Reloading template " + templatePath + " ...");
