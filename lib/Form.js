...
CalipsoForm.prototype.render_fields = function(fields, values) {
  ...
  fields.forEach( function(field) {
    ...
    if(!field.noValue) { // We set this field to a value
      ...
      if(field.name && field.name.split('[').length > 1) {
        ...
        if(values && values[objectName]) {
          ...
          // Just try to get the value via a getter
          if(values && (typeof values.get === "function")) {
            value = values.get(objectName);                        
          }
        }
      }
    ...
    fieldOutput += self.render_field(field, value, values);
 ...
 * @param field
 * @param value
 * @param values - all values (to enable field sets)
 * @returns {String}
 */
CalipsoForm.prototype.render_field = function(field, value, values) {
  ...
  if(field.type == 'fieldset'){
    return self.render_tag_fieldset(field, value, values);
  }
 ...
 * Fieldset
 */
CalipsoForm.prototype.render_tag_fieldset = function(field, value, values) {
  return (
    '<fieldset'+(field.cls?' class="'+field.cls+'"':'')+'>'
    + (field.legend ? '<legend>' + field.legend + '</legend>' : '')
    + '<div class="fieldset-fields">'
    + this.render_fields(field.fields, values)
    + '</div></fieldset>'
  );
};
