...
RenderedBlocks.prototype.set = function(key,value,next) {
  ...
  // calipso.debug("Set " + key + " " + this.contentCache[key]);

  if(this.contentCache[key]) {
    calipso.cache.set(key,value,500,function(err) {
      next();
    });
  } else {
    next();
...
/**
* Load block from content cache
* Unused at the moment, this is in progress
* TODO Ensure blocks are unique based on url
*/
RenderedBlocks.prototype.fromCache = function(key,next) {

  var self = this;
  if(this.contentCache[key]) {
    calipso.cache.check(key,function(err,inCache) {
        if(inCache) {
          calipso.cache.get(key,function(err,item) {
            // Load the request block cache with the cached item
