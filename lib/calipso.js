...
function loadCore(calipso) {
  fs.readdirSync(__dirname + '/core').sort().forEach(function(library) {
    var isLibrary = library.split(".").length > 0 && library.split(".")[1] === 'js',
      requireName = library.split(".")[0],
      libName = requireName.toLowerCase();
    if (isLibrary) calipso[libName] = require(__dirname + '/core/' + requireName);
  });
...
function init(app, initCallback) {
  ...
  calipso.e = new calipso.event.CalipsoEventEmitter({maxListeners: calipso.config.get('server:events:maxListeners')});
