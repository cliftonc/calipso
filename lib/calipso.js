...
var app,
  ...
  lib: {
    ...
    prettySize: require(path.join(rootpath, 'utils/prettySize.js')),
  ...
  calipsoRouter: function(initCallback) {
    ...
    var app = require(path.join(rootpath, 'app')).app;
  ...
  if(res.statusCode === 404 || res.statusCode === 500 || res.statusCode === 200 || res.statusCode === 403) {
    ...
    calipso.theme.render(req, res, function(err, content) {
      ...
      if(err) {
        ...
        calipso.silly("Rendering output ...");
...
function loadModules() {
  ...
  calipso.lib.fs.readdirSync(__dirname + '/../modules').forEach(function(type){
    ...
    if(type != "README" && type != '.DS_Store' && type != '.git' && type != '.svn') {  // Ignore the readme file and .DS_Store file for Macs and possible .git or .svn repo

      calipso.lib.fs.readdirSync(__dirname + '/../modules/' + type).forEach(function(name){

        if(name != "README" && name != '.DS_Store' && type != '.git' && type != '.svn') { // Ignore the readme file and .DS_Store file for Macs and possible .git or .svn repo
...
function requireModule(module, modulePath, reload) {
  ...
  var moduleFile = path.join(rootpath, modulePath + '/' + module.library);
...
function loadThemes(next) {
  ...
  calipso.lib.fs.readdirSync(calipso.lib.path.join(calipso.app.path(),themeBasePath)).forEach(function(folder){
    ...
    if(folder != "README" && folder[0] != '.') {
        var rawThemes = calipso.lib.fs.readdirSync(calipso.lib.path.join(calipso.app.path(),themeBasePath,folder));
        var themes = [];
...
function configureTheme(next, overrideTheme) {
  ...
  if(theme) {
    ...
    require('./Theme').Theme(theme, function(err, loadedTheme) {
