...
function initialiseCalipso(reloadConfig) {
  ...
  // Configure the logging
  configureLogging(calipso.config.get('logging'));      
...
function configureLogging(options) {
  ...
  if(options.file && options.file.enabled) {
    winston.add(winston.transports.File, {level: options.console.level, timestamp: options.file.timestamp, filename: options.file.filepath});
  ...
  if(options.console && options.console.enabled) {
    winston.add(winston.transports.Console, {level: options.console.level, timestamp: options.console.timestamp, colorize: options.console.colorize});
    logMsg += "Console ";
  }
    ...
    require('./Theme').Theme(theme,function(loadedTheme) {
      ...
      if (!calipso.theme) {
        ...
        if(theme.name === defaultTheme) {
          ...
          calipso.error('The `' + themeName + '` theme is missing, attempting to use the default theme: `' + defaultTheme + '`');
