...
calipso = exports = module.exports = {
  ...
  calipsoRouter: function(appReference, config, initCallback) {
    ...
    return function(req,res,next) {
      ...
      res.menu = {admin:new calipso.menu.CalipsoMenu('admin'),
                  adminToolbar:new calipso.menu.CalipsoMenu('adminToolbar'),
...
function doLastModules(req,res,next) {
  ...
  var lastModules = [];
  for(moduleName in calipso.modules) {
...
function doResponse(req,res,next) {
  ...
  if(res.statusCode === 404 || res.statusCode === 500 || res.statusCode === 200) {
    ...
    calipso.theme.render(req, res, function(content) {
      ...
      // Overridable by content / content type
      //if(app.set('environment') === 'production') {
    ...
    if(type != "README") {  // Ignore the readme file
      ...
      calipso.lib.fs.readdirSync(__dirname + '/../modules/' + type).forEach(function(name){
        ...
        if(name != "README") { // Ignore the readme file
              ...
              // Always done so we can get access to about & description
  ...
  require('./Theme').Theme(themeName,function(theme) {
    ...
    calipso.app.stack.forEach(function(middleware,key) {
     if(middleware.handle.tag === 'themeStatic') {
        ...
        var themeStatic = gzip.staticGzip(calipso.app.path + '/themes/' + themeName + '/public');
