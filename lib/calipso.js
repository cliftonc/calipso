...
calipso = exports = module.exports = {
  ...
  cache: require('./calipsoCache').cache(),
  sessionCache: {},
 ...
 * Control the bootstrapping of modules
 */
...
function attachRequestEvents(req, res, next) {
  ...
  req.event = new calipso.lib.event.RequestEventListener();
...
function eventRouteModules(req, res, next) {
  ...
  // Attach our event listener to all modules first
  req = attachRequestEvents(req, res, next);
  ...
  // Route modules
  for(var module in calipso.modules) {
    routeModule(req,res,module,false,next);
  };
 ...
 */
function routeModule(req,res,module,depends,next) {

    // If module is enabled and has no dependencies
    if(calipso.modules[module].enabled && (depends || !calipso.modules[module].fn.depends)) {

      // Fire event
      req.event.modules[module].route_start();

      calipso.modules[module].fn.route(req,res,calipso.modules[module],app,function(err,module) {
        if(err) {
          res.statusCode = 500;
          console.log(err.message);
          res.errorMessage = err.message + err.stack;
        }
        req.event.modules[module].route_finish();
...
function checkAllModulesRouted(req,res,next) {
  ...
  var allRouted = true;
  for(module in req.event.modules) {
    allRouted = (req.event.modules[module].routed || !calipso.modules[module].enabled) && allRouted;
  }
...
function attachModuleEventsAndDependencies() {
  ...
  for(var module in calipso.modules) {
    ...
    calipso.lib.event.addModuleEventListener(calipso.modules[module]);
...
function registerModuleDependencies(module) {
  ...
  if(module.fn && module.fn.depends) {
...
function checkRouted(req,res,moduleName,notifyModuleName) {
  ...
  if(doRoute) {
    // Initiate the module, no req for callback
    // initModule(module.name,true,function() {});
    routeModule(req,res,notifyModuleName,true,function() {});
  }
