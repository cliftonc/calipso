  ...
  calipso.lib.step(             
      ...
      function reloadConfig() {        
        if(res.reloadConfig) {                                                                   
          ...
          initModules(calipso,function() {
            calipso.debug("Modules loaded ...");
            configureTheme(calipso,function() {
              calipso.debug("Theme loaded ...");
              self();
            });                  
          });                  
          ...
          switch(res.statusCode) {
            ...
            case 200:      
              ...
              if(res.format === 'json') {
                ...
                // Render using the theme
                calipso.theme.render(req,res,function(content) {
                  
                  /**
                   * I had massive difficulties getting the asynch
                   * Rendering to work, for some reason send does not 
                   * work, only if i set the connection to close does 
                   * it work at all (?)
                   */                                  
