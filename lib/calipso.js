...
calipso = exports = module.exports = {
   ...
   */
  calipsoRouter: function(initCallback) {

    // Set app and config references
    calipso.app = require(rootpath + 'app').app;
    calipso.config = require(rootpath + 'app').app.set('config');
...
function initialiseCalipso() {
  ...
  // Configure the logging
  configureLogging(calipso.config);

  // Load all the themes
  loadThemes(calipso.config,function() {

    // Initialise the modules and  theming engine
    configureTheme(function() {

      // Load all the modules
      loadModules(calipso.config);
...
function configureLogging(options) {
  ...
  if(options.logs && options.logs.file.enabled) {
    winston.add(winston.transports.File, { level: options.logs.level, timestamp: true, filename: options.logs.file.filepath });
  ...
  if(options.logs && options.logs.console.enabled) {
    winston.add(winston.transports.Console, { level: options.logs.level, timestamp: true, colorize: true });
    logMsg += "Console ";
  }
    ...
    if(type != "README" && type != '.DS_Store') {  // Ignore the readme file and .DS_Store file for Macs
      ...
      calipso.lib.fs.readdirSync(__dirname + '/../modules/' + type).forEach(function(name){
        ...
        if(name != "README" && name != '.DS_Store') { // Ignore the readme file and .DS_Store file for Macs
              ...
              loadAbout(calipso.modules[name], modulePath, 'package.json');
...
function loadAbout(obj, fromPath, file) {
  ...
  if(path.existsSync(packageFile)) {
      ...
      try {
        ...
        console.log("Error parsing " + file + " for " + obj.name + ".");
      }
  };
...
function loadThemes(config,next) {

  // Load the available themes
  calipso.themes = calipso.themes || {};

  calipso.lib.fs.readdirSync(calipso.app.path + '/themes').forEach(function(folder){

    if(folder != "README" && folder != '.DS_Store') {

        var themes = calipso.lib.fs.readdirSync(calipso.app.path + '/themes/' + folder)
        ...
        if(!legacyTheme) {
          themes.forEach(function(theme) {
            if(theme != "README" && theme != '.DS_Store')
              var themePath = calipso.app.path + '/themes/' + folder + '/' + theme + "/";
...
function configureTheme(next, overrideTheme) {
  ...
  if(theme) {
    require('./Theme').Theme(theme,function(loadedTheme) {
      ...
      if (!calipso.theme) {
        ...
        calipso.app.stack.forEach(function(middleware,key) {
         ...
         if(middleware.handle.tag === 'theme.stylus') {
           ...
           var mw = calipso.app.mwHelpers.stylusMiddleware(theme.path);
           calipso.app.stack[key].handle = mw;
         }

         if(middleware.handle.tag === 'theme.static') {
           foundMiddleware = true;
           var mw = calipso.app.mwHelpers.staticMiddleware(theme.path);
           calipso.app.stack[key].handle = mw;
         }
