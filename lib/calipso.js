...
function loadCore(calipso) {
    ...
    fs.readdirSync(rootpath + 'lib/core').forEach(function(library){               
        ...
        if (isLibrary) calipso[libName] = require(rootpath + 'lib/core/' + library);
    });
...
function routingFn() {
  ...
  return function(req, res, next) {
    ...
    res.menu = {admin:new calipso.menu('admin','weight','root',{cls:'admin'}),
                adminToolbar:new calipso.menu('adminToolbar','weight','root',{cls:'admin-toolbar toolbar'}), // TODO - Configurable!
                userToolbar:new calipso.menu('userToolbar','weight','root',{cls:'user-toolbar toolbar'}),
                primary:new calipso.menu('primary','name','root',{cls:'primary'}),
                secondary:new calipso.menu('secondary','name','root',{cls:'secondary'})};
