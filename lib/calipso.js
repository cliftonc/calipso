 ...
 * MIT Licensed
 * 
 * This file contains core components required for calipso to work:
 * 
 * lib : Shortcuts to common libraries (so they dont have to be duplicated in each module)
 * 
 * CalipsoRouter : This function initialises Calipso, and returns a connect
 * middleware function that then deals with requests.
 * 
 */
 ...
 * This iterates through the modules loaded by loadModules (it places them
 *   in an array in the calipso object), and calls the 'init' function
 * exposed by each module (in parallel controlled via step).
 */
...
function loadModules(calipso, options) {
  ...
  for(var module in calipso.modules) {
    ...
    if (calipso.modules[module].enabled && configuredModules[module].enabled == false && typeof calipso.modules[module].fn.disable === 'function') {
      ...
      // TODO - NEED to rethink why this hook exists. 
    ...
    if(type != "README") {  // Ignore the readme file
      ...
      calipso.lib.fs.readdirSync(__dirname + '/../modules/' + type).forEach(function(name){    
        ...
        if(name != "README") { // Ignore the readme file
              ...
              requireModule(calipso, calipso.modules[name], modulePath, name);                
...
/**
 * Load the module through a require, this has been refactored 
 * out into a separate function to enable watching and reloading
 */
function requireModule(calipso, module, modulePath, moduleName) {
  
  module.fn = require(modulePath + '/' + name + '.module');
...
function loadModuleTemplates(calipso, module, moduleTemplatePath) {
  ...
  fs.readdirSync(moduleTemplatePath).forEach(function(name){
    ...
    if(template) {
      templates[name.replace(/\.([^\.]+)$/,'')] = calipso.theme.compileTemplate(template,templatePath,templateExtension);
    }
