...
var Router = function (moduleName, modulePath) {
  ...
  return {
    ...
    addRoute: function (options, next, legacy_options, legacy_next) {
      ...
      // Default options
      var self = this,
          defaults = {end: true, admin: false, user: false, cache: false, permit: null};
      ...
      // Permission Filter
      options.permitFn = new PermissionFilter(path, options.permit);
          ...
          for (i = 0, l = routes.length; i < l; i = i + 1) {
            ...
            if (captures && (!routeMethod || req.method === routeMethod)) {
              ...
              if (route.permit) {
                ...
                if(typeof permit !== "object") permit = {allow: false, msg:'You don\'t have the appropriate permissions to view that page.'};
                if(!permit.allow) {
                  if(!allPages) {
              ...
              if(route.block && cacheBlock && cacheEnabled) {
                ...
                var cacheKey = calipso.cache.getCacheKey(['block',route.block], res.params);
    ...
    routeFn: function(req, res, route, next) {
      ...
      if(typeof route.fn !== "function") console.dir(route);
