...
var Client = module.exports = function Client(options) {
  ...
  this.coreScripts = {
     'jquery': {key:'jquery', url:'jquery-1.6.4.min.js', weight: -100},
...
Client.prototype.addScript = function(options) {
  ...
  if(typeof options === "string") {
    if(this.coreScripts[options]) {
      ...
      options = {name:options, url:options, weight: 0};
    }
  }
  if(!options.name) options.name = options.url;
...
Client.prototype._add = function(arrName, name, options) {
  ...
  // Find first match
  var found = calipso.lib._.find(self[arrName],function(context, value, index, list) {
