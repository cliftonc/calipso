...
CalipsoMenu.prototype.render = function(req) {
  ...
  var renderUp = function(menu) {
      ...
      return self.menuStartTag(menu,selectedClass) + self.menuLinkTag(req,menu);
      
  }
  
  var renderDown = function(menu) {          
      return self.menuEndTag(menu);      
  }
  
  var output = [];  
  self.fnRecurse(self,renderUp,renderDown,output);    
...
CalipsoMenu.prototype.menuStartTag = function(menu,selected) {    
  var menuItemTagId = menu.path.replace(/\//g, '-');    
  return "<li id='" + menuItemTagId + "' class='" + this.name + "-menu-item" + selected + "'>";  
}

CalipsoMenu.prototype.menuLinkTag = function(req,menu) {
 ...
 */
CalipsoMenu.prototype.fnRecurse = function(menu,fnUp,fnDown,output) {
  ...
  if(menu.type === 'root') {   
    ...
    for(var child in menu.children) {
      self.fnRecurse(menu.children[child],fnUp,fnDown,output); 
    }    
  } else {
    // Execute fn      
